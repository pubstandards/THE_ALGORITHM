...
def test_generator():
    ...
    assert next(generator) == date(2024, 9, 12)

    next_10_years = [next(generator) for _ in range(10 * 12)]
    for ps_date in next_10_years:
        # Check we are in fact on Thursday
        assert ps_date.weekday() == 3


def test_offset_from_date():
    assert ps_offset_from_date(date(2005, 12, 15)) == 1
    assert ps_offset_from_date(date(2006, 1, 12)) == 2

    with pytest.raises(ValueError):
        # Before the Pub Standards era
        ps_offset_from_date(date(2005, 1, 1))

    with pytest.raises(ValueError):
        # Not a Pub Standards date
        ps_offset_from_date(date(2005, 12, 16))

    # Final date before the COVID hiatus
    assert ps_offset_from_date(date(2020, 2, 13)) == 171

    with pytest.raises(ValueError):
        # During the COVID hiatus
        ps_offset_from_date(date(2020, 3, 12))

    # First date after COVID hiatus
    assert ps_offset_from_date(date(2024, 9, 12)) == 172


def test_from_offset():
    assert ps_date_from_offset(1) == date(2005, 12, 15)
    assert ps_date_from_offset(2) == date(2006, 1, 12)
    # Final date before the COVID hiatus
    assert ps_date_from_offset(171) == date(2020, 2, 13)
    # First date after COVID hiatus
    assert ps_date_from_offset(172) == date(2024, 9, 12)
